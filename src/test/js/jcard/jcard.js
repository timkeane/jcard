...
QUnit.module('jcard.Builder', {
	beforeEach: function(assert){
		this.BUILDER = new jcard.Builder();
	},
...
QUnit.test('constructor', function(assert){
	assert.expect(1);
	assert.deepEqual(this.BUILDER.data, ['vcard', [['version', {}, jcard.Data.TEXT, '4.0']]]);
});
...
QUnit.test('name', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.NAME, {}, jcard.Data.TEXT, ['House', 'Gregory', 'A.', 'Dr.', 'MD']]);
	...
	assert.deepEqual(prop, [jcard.Property.NAME, {}, jcard.Data.TEXT, ['House', 'Gregory', '', '', '']]);
});
...
QUnit.test('address', function(assert){
	...
	var address = {
		...
		zip: '11201',
		country: 'U.S.A.',
		type: jcard.Type.WORK
	};
	
	var prop = this.BUILDER.address(address);
	
	assert.deepEqual(prop, [jcard.Property.ADDRESS, {type: jcard.Type.WORK, label: '2 Metrotect Ctr.\\n4th floor\\nBrooklyn\\, NY 11201\\nU.S.A.'}, jcard.Data.TEXT, ['2 Metrotect Ctr.', 'Brooklyn', '4th floor', 'NY', '11201', 'U.S.A.']]);
	...
	address = {
		...
		state: 'NY',
		zip: '11201',
		type: jcard.Type.HOME
	};
	
	prop = this.BUILDER.address(address);
	
	assert.deepEqual(prop, [jcard.Property.ADDRESS, {type: jcard.Type.HOME, label: '2 Metrotect Ctr.\\nBrooklyn\\, NY 11201'}, jcard.Data.TEXT, ['2 Metrotect Ctr.', 'Brooklyn', '', 'NY', '11201', '']]);
	...
	assert.deepEqual(prop, [jcard.Property.ADDRESS, { label: '2 Metrotect Ctr.\\nBrooklyn\\, NY 11201'}, jcard.Data.TEXT, ['2 Metrotect Ctr.', 'Brooklyn', '', 'NY', '11201', '']]);
});
...
QUnit.test('email', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.EMAIL, {type: jcard.Type.WORK}, jcard.Data.TEXT, 'me@work.com']);
	...
	assert.deepEqual(prop, [jcard.Property.EMAIL, {type: jcard.Type.HOME}, jcard.Data.TEXT, 'you@home.com']);
	...
	assert.deepEqual(prop, [jcard.Property.EMAIL, {}, jcard.Data.TEXT, 'you@home.com']);
});
...
QUnit.test('phone', function(assert){
	...
	var phone = {
		...
		number: '555-1212',
		extension: '234',
		type: [jcard.Type.WORK, jcard.Type.CELL]
	};
	
	var prop = this.BUILDER.phone(phone);
	
	assert.deepEqual(prop, [jcard.Property.PHONE, {type: [jcard.Type.WORK, jcard.Type.CELL]}, jcard.Data.URI, '+1-212-555-1212,234']);
	...
	assert.deepEqual(prop, [jcard.Property.PHONE, {type: jcard.Type.HOME}, jcard.Data.URI, '+1-212-555-1212']);
	...
	assert.deepEqual(prop, [jcard.Property.PHONE, {}, jcard.Data.URI, '+1-212-555-1212']);
});
...
QUnit.test('url', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.URL, {type: jcard.Type.WORK}, jcard.Data.URI, 'http://maps.nyc.gov/']);
	...
	assert.deepEqual(prop, [jcard.Property.URL, {type: jcard.Type.HOME}, jcard.Data.URI, 'http://me.home.com/']);
	...
	assert.deepEqual(prop, [jcard.Property.URL, {}, jcard.Data.URI, 'http://me.home.com/']);
});
...
QUnit.test('organization', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.ORGANIZATION, {type: jcard.Type.WORK}, jcard.Data.TEXT, 'My Fancy Company']);
	...
	assert.deepEqual(prop, [jcard.Property.ORGANIZATION, {type: 'Fancy'}, jcard.Data.TEXT, 'My Fancy Company']);
	...
	assert.deepEqual(prop, [jcard.Property.ORGANIZATION, {}, jcard.Data.TEXT, 'My Fancy Company']);
});
...
QUnit.test('timezone', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.TIMEZONE, {}, jcard.Data.UTC_OFFSET, '-5:00']);
});
...
QUnit.test('geo', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.GEOGRAPHY, {type: jcard.Type.WORK}, jcard.Data.URI, 'geo:' + geo.lat + ',' + geo.lng]);
});
...
QUnit.test('note', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.NOTE, {}, jcard.Data.TEXT, 'This is my note right here!']);
});
...
QUnit.test('revision', function(assert){
	...
	assert.deepEqual(prop, [jcard.Property.REVISION, {}, jcard.Data.DATE, rev]);
});
...
QUnit.test('formattedName (has name)', function(assert){
	...
	assert.deepEqual(props[props.length - 1], [jcard.Property.FORMATTED_NAME, {}, jcard.Data.TEXT, 'Dr. Gregory A. House, MD'])
});
...
QUnit.test('formattedName (no name, has organization)', function(assert){
	...
	assert.deepEqual(props[props.length - 1], [jcard.Property.FORMATTED_NAME, {}, jcard.Data.TEXT, 'My Fancy Company'])
});
...
QUnit.test('formattedName (no name, no organization)', function(assert){
	...
	$.each(this.BUILDER.data[1], function(_, prop){
		assert.notEqual(prop[0], jcard.Property.FORMATTED_NAME);
	});
});
